...
import com.grgbanking.counter.common.core.util.SpringContextHolder;
...
public class GrgTokenDealServiceImpl {
	 ...
	 * @return R
	 */
	public Resp<Boolean> removeToken(String token) {

		OAuth2AccessToken accessToken = tokenStore.readAccessToken(token);
		if (accessToken == null || StrUtil.isBlank(accessToken.getValue())) {
			return Resp.success(Boolean.TRUE, "退出失败，token 无效");
		}
	 ...
	 * @return R
	 */
	public Resp queryTokenByUsername(Page page, String username) {
		String key = SecurityConstants.GRG_PREFIX + SecurityConstants.OAUTH_PREFIX + "uname_to_access_z:";
	 ...
	 * @return
	 */
	public Resp<Page> queryToken(Page page) {
		// 根据分页参数获取对应数据
		String key = SecurityConstants.GRG_PREFIX + SecurityConstants.OAUTH_PREFIX + "access:*";
