 ...
 */
public class GrgRedisTokenStore implements TokenStore {
	...
	public GrgRedisTokenStore(RedisConnectionFactory connectionFactory) {
	...
	public long doMaintenance() {
		...
		try (RedisConnection conn = getConnection()) {
			...
			Cursor<byte[]> clientToAccessKeys = conn.scan(ScanOptions.scanOptions()
					.match(prefix + CLIENT_ID_TO_ACCESS + "*").build());
			...
			Cursor<byte[]> unameToAccessKeys = conn.scan(ScanOptions.scanOptions()
					.match(prefix + UNAME_TO_ACCESS + "*").build());
